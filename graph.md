...
&emspu=Q.front();<br>
&emspQ.pop();<br>
&emsp当u为源顶点或者在顶点中存在比u的level少1的顶点v时:<br>
&emsp&emsp向v推进最大流（c(u,v)）；（u点的R值减少c(u,v)，v点的R值增加c(u,v)）<br>
&emsp&emsp如果推进之前的R[u]<c(u,v)，则推进的流值为R[u]<br>
&emsp&emsp响应的反向弧值增加，正向弧值减少（c(u,v)减少推进流值，c(v,u)增加推进的流值）<br>
&emsp&emsp如果v点是汇点t，则全局的最大流量值（所求的结果）+=推进的流量值<br>
&emsp&emsp如果v不是源顶点s也不是汇点t，则Q.push(v)<br>
&emsp如果u不是s也不是t而且R[u]还有残余量：<br>
&emsp&emsp把u的level加1，压入到Q中。<br>
  end while<br>
4.结束。<br>
